...
def main(dbfds):
  ...
  #try:
    ...
    ##   1) an arbitrary specified layer
    ##   2) any layers with the names (see definitions above):
    ##      badLabels, blankPolys, excessContacts, idCAF
    editLayers = [mup,badLabels,badPolys,blankPolys,excessContacts,idCAF]
    savedLayers = []
    if sys.argv[3] <> "#":
        editLayers = [sys.argv[3]]+editLayers
    for aLyr in editLayers:
        try:
            ...
            layerName = arcpy.Describe(lyr).nameString
            addMsgAndPrint('A')
            if layerName.find('\\') > 0:
                groupLyrName = layerName[:layerName.find('\\')]
            else:
                groupLyrName = None
            addMsgAndPrint('B')
            ...
            lyrPath = os.path.join(docsPath, layerName + '.lyr')
            ...
            addMsgAndPrint('D')
            savedLayers.append([lyrPath,df,refLyr,insertPos,lyr,groupLyrName])
            addMsgAndPrint('Layer '+aLyr+' saved to '+lyrPath+' and removed from map composition')
            addMsgAndPrint('E')
        except:
            addMsgAndPrint('did not find layer '+aLyr)
            pass
    ...
    addMsgAndPrint(sqlQuery)
    arcpy.MakeFeatureLayer_management(mup,mupLayer2,sqlQuery)
    ...
    if saveMUP:
        ...
        try:
            ...
            raise arcpy.ExecuteError
    addMsgAndPrint("  deleting " + mupPath)
    ...
    addMsgAndPrint('  getting '+xxPolys+' rows')
    ...
    while row:
        if row.getValue(OidName)in badPolyList:
            ...
            rows.updateRow(row)
        row = rows.next()
    addMsgAndPrint('  getting '+xxLabels+' rows')
    ...
    for savedLayer in savedLayers:
        lyrPath,dataFrame,refLyr,insertPos,lyr,groupLyrName = savedLayer
        addMsgAndPrint('    layer '+lyr.name)
        addLyr = arcpy.mapping.Layer(lyrPath)
        if groupLyrName == None:
            arcpy.mapping.AddLayer(dataFrame, addLyr)
            arcpy.mapping.InsertLayer(dataFrame, refLyr, addLyr, insertPos)
        else:
            groupLyr = findLyr(groupLyrName)[0]
            addMsgAndPrint('    '+str(groupLyr))
            arcpy.mapping.AddLayerToGroup(dataFrame,groupLyr,addLyr)
            addMsgAndPrint('   added layer to group '+groupLyrName)
