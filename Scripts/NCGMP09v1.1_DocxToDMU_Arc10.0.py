...
versionString = 'NCGMP09v1.1_DocxToDMU_Arc10.0.py, version of 17 September 2012'
debug = False
...
for n in range(1,len(msRows)):
    ...
    if rankParaStylesAreEqual(hKeyStyleDict[str(lastHKey)],row[4]): # is sibling to lastHKey
        ...
        if maxHKeyElementLength > lastElement:
            maxHKeyElementLength = lastElement
...
elemLength = len(str(maxHKeyElementLength))
for n in range(len(msRows)):
    newHKey = []
    for element in hKeys[n]:
        newHKey.append(str(element).zfill(elemLength))  
